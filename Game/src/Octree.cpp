...
void Octree::create(const ofMesh & geo, int numLevels) {
    ...
    colors.push_back(ofColor::hotPink);
    colors.push_back(ofColor::salmon);
    colors.push_back(ofColor::darkGrey);
...
void Octree::subdivide(const ofMesh & mesh, TreeNode & node, int numLevels, int level) {
    ...
    for (int i =0; i<boxList.size(); i++) {
        ...
        int points = getMeshPointsInBox(mesh, node.points, boxList[i], child.points);
        if(points) {
...
bool Octree::intersect(const Ray &ray, const TreeNode & node, TreeNode & nodeRtn) {
    ...
    // Check if the ray intersect the current node's box
    // if we hit a leaf node, we 'selected' a point and should return that node
    if (node.box.intersect(ray, -1000, 1000)) {
        ...
        if (node.children.size() == 0) {
            ...
            // recursivly check to see if the ray intersects with the node's children
