...
void ImpulseRadialForce::setMag(float mag) {
    magnitude=mag;
}
float ImpulseRadialForce::getMag() { return magnitude;}

DiskRadialForce::DiskRadialForce(float mag, float h) : ImpulseRadialForce(mag) {
    height=h;
}

void DiskRadialForce::setHeight(float h) {
    height=h;
}

void DiskRadialForce::updateForce(Particle * particle) {

    // we basically create a random direction for each particle
    // the force is only added once after it is triggered.
    //
    ofVec3f dir = ofVec3f(ofRandom(-1, 1), ofRandom(-height, height), ofRandom(-1, 1));
    particle->forces += dir.getNormalized() * ofRandom(0,getMag());
}
