 ...
 //     assert(min < max);
	...
	//
	 bool overlap(const Box &box) {
         return (min().x() < box.parameters[1].x() && box.parameters[0].x() < max().x() && min().y() < box.parameters[1].y() && box.parameters[0].y() < max().y() && min().z() < box.parameters[1].z() && box.parameters[0].z() < max().z());
	}
